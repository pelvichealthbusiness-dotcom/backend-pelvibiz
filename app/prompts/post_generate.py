...
def build_masterclass_person_prompt(brand: dict) -> str:
    ...
    return (
        f"Full body professional photograph of {outfit_face}. "
        f"Style: {identity}, confident and approachable expression, slight smile. "
        f"Framing: FULL BODY from head to toe, standing, centered, VERTICAL PORTRAIT orientation (tall format, 2:3 aspect ratio). "
...
def build_masterclass_face_mode_prompt(brand: dict) -> str:
    ...
    return (
        ...
        f"You MUST preserve EXACTLY: this person's face, gender, age, skin tone, and physical features. "
        f"CRITICAL: DO NOT change the gender — if the reference shows a MAN, generate a MAN; if a WOMAN, generate a WOMAN. "
        f"Generate a full-body professional photograph of this EXACT person. "
        f"Outfit: {outfit}. "
        f"Style: {identity}, confident posture, natural smile, standing or slight three-quarter pose. "
        f"Framing: FULL BODY from head to toe, centered, VERTICAL PORTRAIT orientation (tall format, 2:3 aspect ratio). "
        f"Background: solid white or very light neutral — NO complex backgrounds, NO furniture, NO props. "
